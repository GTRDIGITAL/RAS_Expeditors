...
Poți schimba parola oricând din Contul tău, secțiunea Schimbă parola.
...
GTR Digital'''
    ...
    return render_template("create-user.html", email=current_user)
...
def change_password():
    if request.method == 'POST':
        ...
        if not check_password_hash(current_user.password, current_password):
            flash('Parola curentă este incorectă.', category='error')
            return redirect(url_for('auth.change_password'))

        # Verifică dacă noile parole coincid
        if new_password != confirm_password:
            flash('Parolele nu coincid!', category='error')
        elif len(new_password) < 8:  # Poți ajusta lungimea minimă
            flash('Parola trebuie să aibă cel puțin 8 caractere.', category='error')
        else:
            ...
            db.session.commit()
            flash('Parola a fost schimbată cu succes!', category='success')
            return redirect(url_for('views.main'))
